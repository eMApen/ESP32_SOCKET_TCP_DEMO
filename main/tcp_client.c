    ...
    while (1) {
        ...
        while (1) { //这个while里面的全部都是发送函数
            ...
            // Data received
                ...
                if(ret!=NULL){
                    // int err = send(sock, "PIC", 3, 0);
                    // ESP_LOGI(TAG, "Pic server required from %s:,nreply", host_ip);
                    if (err < 0) {
                        ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
                        break;
                    }
                    ...
                    {   
                        int len = recv(sock, rx_buffer, sizeof(rx_buffer) - 1, 0);
                            // Error occurred during receiving
                            ESP_LOGE(TAG, "recv : %s, len= %d",rx_buffer,len );
                            if (len < 0) {
                                ESP_LOGE(TAG, "recv failed: errno %d", errno);
                                break;
                            }
                            sprintf(rd,"Ready%d",i+1);
                            ret = strstr(rx_buffer,rd);
                            if(len>=10&&ret!=NULL){
                                flage=len%6;
                            }
                            else if(len<=9&&ret!=NULL){
                                flage=1;
                            }
                            else{
                                flage=-1;
                        ...
                        // int k=0;
                        if(flage>=1){
                            int xlen = strlen(cat[i]);
                            ESP_LOGI(TAG, "PIC LENTH = %d NUM= %d",xlen,i);
                            err = send(sock, cat[i], xlen, 0);
                            if (err < 0) {
                                ESP_LOGE(TAG, "Error occurred during sending: errno %d", errno);
                                break;
                            }
